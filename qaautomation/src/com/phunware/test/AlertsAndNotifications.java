...
public class AlertsAndNotifications extends Suite {
	...
	public void validationOfSortingOnDateField() {
		...
		SeleniumUtils.sleepThread(4);
		logger.info("Verify Sorting Up functionality on Date field");
		// Identify Messages First Pagination link
		element = SeleniumUtils.waitForElementToIdentify(
				Suite.objectRepositoryMap.get(
						"AlertsNotificationsMessagesTabMessagesFirstLink")
						.getLocatortype(),
				Suite.objectRepositoryMap.get(
						"AlertsNotificationsMessagesTabMessagesFirstLink")
						.getLocatorvalue());
		SeleniumUtils.clickOnElement(element);
		SeleniumUtils.sleepThread(3);
		logger.info("Click on Date hedaer ");
		// Identify Date header
		element = SeleniumUtils
				.waitForElementToIdentify(
						Suite.objectRepositoryMap
								.get("AlertsNotificationsMessagesTabMessagesTableHeaderRowDateFilter")
								.getLocatortype(),
						Suite.objectRepositoryMap
								.get("AlertsNotificationsMessagesTabMessagesTableHeaderRowDateFilter")
								.getLocatorvalue());
		...
		SeleniumUtils.sleepThread(5);
		logger.info("Verify if the newly created message ["
				+ testcaseArgs.get("messageText")
				+ "] displayed first in the messgaes table");
		// Identify First Row data
		element = SeleniumUtils.waitForElementToIdentify(
				Suite.objectRepositoryMap.get(
						"AlertsNotificationsMsgTabMsgTableFirstRowMsgData")
						.getLocatortype(),
				Suite.objectRepositoryMap.get(
						"AlertsNotificationsMsgTabMsgTableFirstRowMsgData")
						.getLocatorvalue());
		// Get the text of the message
		String RetMessage = SeleniumUtils.getText(element);
		isTextMatching = SeleniumUtils.assertEqual(RetMessage,
				testcaseArgs.get("messageText"));
		if (!isTextMatching) {
			logger.error("Sorting functionality fails on 'Date/Time' header in Messages table. "
					+ "The expected message displayed at first is ["
					+ testcaseArgs.get("messageText")
					+ "] and the actual return message is [" + RetMessage + "]");
			ReportUtils
					.setStepDescription(
							"Sorting functionality fails on 'Date/Time' header in Messages table",
							"", testcaseArgs.get("messageText"), RetMessage,
							true);
			m_assert.fail("Sorting functionality fails on 'Date/Time' header in Messages table. "
					+ "The expected message displayed at first is ["
					+ testcaseArgs.get("messageText")
					+ "] and the actual return message is [" + RetMessage + "]");
		}
	...
	public void validationOfSortingOnMessagesField() {
		...
		logger.info("Verify the Sorting down functionality on Messages field");
		// Identify Date/Time header
		element = SeleniumUtils
				.findobject(
						Suite.objectRepositoryMap
								.get("AlertsNotificationsMessagesTabMessagesTableHeaderRowDateFilter")
		...
		SeleniumUtils.clickOnElement(element);
		SeleniumUtils.sleepThread(2);
		logger.info("Click on Message hedaer ");
		// Identify Messages header
		element = SeleniumUtils
				.waitForElementToIdentify(
						Suite.objectRepositoryMap
								.get("AlertsNotificationsMessagesTabMessagesTableHeaderRowMessageFilter")
								.getLocatortype(),
						Suite.objectRepositoryMap
								.get("AlertsNotificationsMessagesTabMessagesTableHeaderRowMessageFilter")
								.getLocatorvalue());
		SeleniumUtils.clickOnElement(element);
		SeleniumUtils.sleepThread(4);
		logger.info("Verify messages are displayed as per alphabet format");
		...
		element = SeleniumUtils.waitForElementToIdentify(
				...
				Suite.objectRepositoryMap.get(
						"AlertsNotificationsMessagesTabMessagesTable")
						.getLocatorvalue());
		// Get number of messages in a table
		int countOfMsgs = SeleniumUtils.getCountFromTable(element);
		// If count is less than 2
		if (countOfMsgs < 2) {
			logger.warn("Unable to perform Sorting down functionality as "
					+ "total number of messages are displayed is ["
					+ countOfMsgs + "]");
			ReportUtils.setStepDescription(
					"Unable to perform Sorting down functionality as "
							+ "total number of messages are displayed is ["
							+ countOfMsgs + "]", true);

		} else {
			// count is greater than 2
			// check for soring down functionality
			boolean isChecked = SeleniumCustomUtils
					.checkSortingDownOnMessages(element);
			if (!isChecked) {
				logger.error("Sortting down functionality fails on Messages ");
				ReportUtils.setStepDescription(
						"Sortting down functionality fails on Messages", true);
				m_assert.fail("Sorting down functionality fails on Messages");
			}
			logger.info("Sorting down functionality successful on Messages");
			logger.info("Verify Sorting up functionality on Messages");
			element = SeleniumUtils
					.findobject(
							Suite.objectRepositoryMap
									.get("AlertsNotificationsMessagesTabMessagesTableHeaderRowMessageFilter")
							...
							Suite.objectRepositoryMap
									...
									.getLocatorvalue());
			if (element == null) {
				logger.error("Unable to identify manual Messages table header Messages filed");
				ReportUtils
						.setStepDescription(
								"Unable to identify manual Messages table header Messages filed",
								true);
				m_assert.fail("Unable to identify manual Messages table Messages filed");
			}
			// click on Messages header
			SeleniumUtils.clickOnElement(element);
			SeleniumUtils.sleepThread(3);
			element = SeleniumUtils.waitForElementToIdentify(
					Suite.objectRepositoryMap.get(
					...
					Suite.objectRepositoryMap.get(
							"AlertsNotificationsMessagesTabMessagesTable")
							.getLocatorvalue());
			// check the sorting up functionality
			isChecked = SeleniumCustomUtils.checkSortingUpOnMessages(element);
			if (!isChecked) {
				logger.error("Sort Up functionality fails on Messages ");
				ReportUtils.setStepDescription(
						"Sort Up functionality fails on Messages ", true);
				m_assert.fail("Sort Up functionality fails on Messages");
			}
		}
		m_assert.assertAll();
	}
	...
	public void verifyMessagesLayout() {
		...
		// Identify Messages length dropdown
		element = SeleniumUtils.findobject(
				Suite.objectRepositoryMap.get(
	...
	public void clickOnMessage() {
		...
		element = SeleniumUtils.findobject(
				...
				Suite.objectRepositoryMap.get(
						...
						.getLocatorvalue());
		for (int i = 1; i <= 20; i++) {
			isClicked = SeleniumCustomUtils.clickAtMessageInMessageOverview(
					element, testcaseArgs.get("message"));
			if (isClicked) {
				break;
			}
			SeleniumUtils.click(
	...
	public void verifySegmentsLayout() {
		...
		SeleniumUtils
				.click(Suite.objectRepositoryMap.get(
						"AlertsNotificationsSegmentsTabText").getLocatortype(),
						Suite.objectRepositoryMap.get(
								"AlertsNotificationsSegmentsTabText")
								.getLocatorvalue());
	...
	public void createNewSegment() {
		...
		if (!isTextMatching) {
			...
			isTextMatching = SeleniumUtils.assertEqual(
					...
					Suite.objectRepositoryMap.get(
							"AlertsNotificationsSegmentsTabText").getExptext());
			if (!isTextMatching) {
				logger.error("[Segments] tab header text matching failed: The expected text is ["
						+ Suite.objectRepositoryMap.get(
								"AlertsNotificationsSegmentsTabText")
								.getExptext()
						+ "] and the actual return text is ["
				...
				m_assert.fail("[Segments] tab header text matching failed: The expected text is ["
						+ Suite.objectRepositoryMap.get(
								"AlertsNotificationsSegmentsTabText")
								.getExptext()
						+ "] and the actual return text is ["
		...
		logger.info("Segment creation is successful : Created Segment is displayed in Segments list");
	...
	public void editSegment() {
		...
		boolean isEdited = SeleniumCustomUtils
				.clickAtEditOfSegmentInSegmentList(element,
						testcaseArgs.get("segment"),
		...
		boolean isSegmentPresent = SeleniumCustomUtils
				.verifySegmentInSegmentList(element,
						testcaseArgs.get("segmentNewName"));
		if (!isSegmentPresent) {
			logger.error("Segment edition fails : Segment is not present in Segment List");
			ReportUtils
					.setStepDescription(
							"Segment edition fails : Segment is not present in Segment List",
							true);
			m_assert.fail("Segment edition fails : Segment is not present in Segment List");
		}
	...
	public void deleteSegment() {
		...
		if (configproperties.get(0).equalsIgnoreCase("SAFARI")) {
			...
			isClicked = SeleniumCustomUtils.deleteSegmentInSegmentList(element,
					testcaseArgs.get("segment"));
			if (!isClicked) {
