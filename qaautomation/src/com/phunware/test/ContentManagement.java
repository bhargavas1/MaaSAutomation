...
public class ContentManagement extends Suite {
	...
	public void validationsOnCreateNewSchema() {
		...
		if (!isTextMatching) {
			...
			SeleniumUtils.isElementPresentHavingText(
					Suite.objectRepositoryMap.get(
							"ContentManagementSchemasTabHeaderText")
							.getLocatortype(),
					Suite.objectRepositoryMap.get(
		...
		element = SeleniumUtils.findobject(
				...
				Suite.objectRepositoryMap.get(
						...
						.getLocatorvalue());
		// Get the text
		String FieldsErrorMessage = SeleniumUtils.getText(element);
		isTextMatching = SeleniumUtils.assertEqual(
				FieldsErrorMessage,
				Suite.objectRepositoryMap.get(
						"CMSchemasTabCreateNewSchemaFieldsErrorMSG")
	...
	public void deleteSchema() {
		...
		isTextMatching = SeleniumUtils.assertEqual(
				...
				Suite.objectRepositoryMap.get(
						...
						.getExptext());
		if (!isTextMatching) {
			logger.error("'Schema' success message text matching failed. The Expected text is ["
					+ Suite.objectRepositoryMap.get(
							"CMSchemasTabCreateNewSchemaDeleteSuccessMSG")
							.getExptext()
					+ "] and the actual return text is ["
			...
			ReportUtils.setStepDescription(
					...
					Suite.objectRepositoryMap.get(
							"CMSchemasTabCreateNewSchemaDeleteSuccessMSG")
							.getExptext(), SchemaSuccessMessage, true);
			m_assert.fail("'Schema' success message text matching failed. The Expected text is ["
					+ Suite.objectRepositoryMap.get(
							"CMSchemasTabCreateNewSchemaDeleteSuccessMSG")
							.getExptext()
					+ "] and the actual return text is ["
	...
	public void createNewComplexSchema() {
		...
		SeleniumUtils.clickOnElement(element);
		SeleniumUtils.sleepThread(5);
		// Identify Attribute fields set
		element = SeleniumUtils.waitForElementToIdentify(
				Suite.objectRepositoryMap.get(
						"CMSchemasTabCreateNewSchemaAddAttributeSet")
						.getLocatortype(),
				Suite.objectRepositoryMap.get(
						"CMSchemasTabCreateNewSchemaAddAttributeSet")
						.getLocatorvalue());
		if (element == null) {
			logger.error("Unable to identify 'ATTRIBUTE' set in "
					+ "[CREATE NEW SCHEMA] page");
			ReportUtils.setStepDescription(
					"Unable to identify 'ATTRIBUTE' set "
							+ "in [CREATE NEW SCHEMA] page", true);
			m_assert.fail("Unable to identify 'ATTRIBUTE' set "
					+ "in [CREATE NEW SCHEMA] page");
		}
		logger.info("Click operation on 'ADD ATTRIBUTE' button is successful. "
				+ "Enter data in 'Key' and 'Val' fields");
	...
	public void createNewContainer() {
		...
		element = SeleniumUtils.waitForElementToIdentify(
				...
				Suite.objectRepositoryMap.get(
						...
						.getLocatorvalue());
		// Get the text
		String NewContainerSuccessMsg = SeleniumUtils.getText(element);
		isTextMatching = SeleniumUtils.assertEqual(
		...
		isTextMatching = SeleniumUtils
				.assertEqual(
						...
						Suite.objectRepositoryMap
								...
								.getExptext());
		if (!isTextMatching) {
			logger.error("'Delete Container success msge' text matching failed. The Expected text is ["
					+ Suite.objectRepositoryMap
							.get("ContentManagementStructureTabNewContainerDeleteSuccessMsg")
							.getExptext()
					+ "] and the actual return text is ["
			...
			ReportUtils
					.setStepDescription(
							...
							Suite.objectRepositoryMap
									.get("ContentManagementStructureTabNewContainerDeleteSuccessMsg")
									.getExptext(), NewContainerSuccessMsg, true);
			m_assert.fail("'Delete Container success msge' text matching failed. The Expected text is ["
					+ Suite.objectRepositoryMap
							.get("ContentManagementStructureTabNewContainerDeleteSuccessMsg")
							.getExptext()
					+ "] and the actual return text is ["
