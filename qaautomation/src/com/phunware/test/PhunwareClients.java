...
public class PhunwareClients extends Suite {
	...
	public void validationInCreateUser() {
		...
		SeleniumUtils.clickOnElement(element);
		SeleniumUtils.sleepThread(1);
		// Identify Invalid Email Element
		element = SeleniumUtils
				.waitForElementToIdentify(
						Suite.objectRepositoryMap.get(
								"ClientCreateNewUserInvalidEmailMSG")
								.getLocatortype(), Suite.objectRepositoryMap
								.get("ClientCreateNewUserInvalidEmailMSG")
								.getLocatorvalue());
		// Get the text
		String invalidEmailMsg = SeleniumUtils.getText(element);
		// Get the exp text
		...
		element = SeleniumUtils.waitForElementToIdentify(
				...
				Suite.objectRepositoryMap.get(
						...
						.getLocatorvalue());
		// Get the text
		String invalidPasswordMsg = SeleniumUtils.getText(element);
		// Get the exp text
		...
		isTextMatching = SeleniumUtils.assertEqual(headerText, ExpheaderText);
		if (!isTextMatching) {
			logger.error("Validation failed at First and Last Name fields in [Add User]: User can"
					+ " create new user account when user enters white space in First and Last Name fields");
			ReportUtils
					.setStepDescription(
							"Validation failed at First and Last Name fields in [Add User]: User can"
									+ " create new user account when user enters white space in First and Last Name fields",
							true);
			m_assert.fail("Validation failed at First and Last Name fields in [Add User]: User can"
					+ " create new user account when user enters white space in First and Last Name fields");
			...
			element = SeleniumUtils.waitForElementToIdentify(
					...
					Suite.objectRepositoryMap.get("ClientDeleteUserSuccessMSG")
							.getLocatorvalue());
			// Get the text
			String delUserSuccessMsg = SeleniumUtils.getText(element);
			// Get the exp text
			...
			if (!isTextMatching) {
				logger.error("Delete User success message text matching failed. The Expected text is ["
						+ ExpdelUserSuccessMsg
						+ "] and the return text is ["
						+ delUserSuccessMsg + "]");
				ReportUtils.setStepDescription(
				...
				m_assert.fail("Delete User success message text matching failed. The Expected text is ["
						+ ExpdelUserSuccessMsg
						+ "] and the return text is ["
						+ delUserSuccessMsg + "]");
			}
	...
	public void createUser() {
		...
		if (!Message.equalsIgnoreCase(ErrorMessage)) {
			...
			if (!isTextMatching) {
				logger.error("New User success message text matching failed. The Expected text is ["
						+ SuccessMessage
						+ "] and the return text is ["
						+ Message + "]");
				ReportUtils.setStepDescription(
				...
				m_assert.fail("New User success message text matching failed. The Expected text is ["
						+ SuccessMessage
						+ "] and the return text is ["
						+ Message + "]");
			}
	...
	public void editUser() {
		...
		boolean isClicked = SeleniumCustomUtils.clickAtEditForUsers(element,
				testcaseArgs.get("username"));
		SeleniumUtils.sleepThread(4);
		if (!isClicked) {
		...
		element = SeleniumUtils.waitForElementToIdentify(
				...
				Suite.objectRepositoryMap.get("ClientCreateNewUserSuccessMSG")
						.getLocatorvalue());
		// Get text
		String userSuccessMsg = SeleniumUtils.getText(element);
		// Get exp text
		...
		isTextMatching = SeleniumUtils.assertEqual(userSuccessMsg,
				ExpuserSuccessMsg);
		if (!isTextMatching) {
			logger.error("Edit User success message text matching failed. The Expected text is ["
					+ ExpuserSuccessMsg
					+ "] and the return text is ["
					+ userSuccessMsg + "]");
			ReportUtils.setStepDescription(
			...
			m_assert.fail("Edit User success message text matching failed. The Expected text is ["
					+ ExpuserSuccessMsg
					+ "] and the return text is ["
					+ userSuccessMsg + "]");
		}
	...
	public void validationInCreateRole() {
		...
		element = SeleniumUtils.waitForElementToIdentify(
				...
				Suite.objectRepositoryMap.get(
						...
						.getLocatorvalue());
		// Get the text
		String NewRoleErrorMsg = SeleniumUtils.getText(element);
		// Get the exp text
		...
		isTextMatching = SeleniumUtils.assertEqual(NewRoleErrorMsg,
				ExpNewRoleErrorMsg);
		if (!isTextMatching) {
			logger.error("Error message while creating new Role, text matching failed. Expected text is ["
					+ ExpNewRoleErrorMsg
					+ "] and the return text is ["
					+ NewRoleErrorMsg + "]");
			ReportUtils
			...
			m_assert.fail("Error message while creating new Role, text matching failed. Expected text is ["
					+ ExpNewRoleErrorMsg
					+ "] and the return text is ["
					+ NewRoleErrorMsg + "]");
		}
	...
	public void createRole() {
		...
		element = SeleniumUtils.waitForElementToIdentify(
				...
				Suite.objectRepositoryMap.get("ClientCreateRoleSuccessMSG")
						.getLocatorvalue());
		// Get the text
		String NewRoleSuccessMsg = SeleniumUtils.getText(element);
		// Get the exp text
	...
	public void editRole() {
		...
		element = SeleniumUtils.findobject(
				...
				Suite.objectRepositoryMap.get("ClientUserRolesRolesListTable")
						.getLocatorvalue());
		// Edit the Role
		boolean isRoleEdited = SeleniumCustomUtils.editRole(element,
	...
	public void deleteRole() {
		...
		element = SeleniumUtils.waitForElementToIdentify(
				...
				Suite.objectRepositoryMap.get("ClientDeleteRoleSuccessMSG")
						.getLocatorvalue());
		// Get the text
		String delRoleSuccessMsg = SeleniumUtils.getText(element);
		// Get the exp text
		...
		if (!isTextMatching) {
			logger.error("Role deletion message text matching failed. Expected text is ["
					+ ExpdelRoleSuccessMsg
					+ "] and the return text is ["
					+ delRoleSuccessMsg + "]");
			ReportUtils.setStepDescription(
			...
			m_assert.fail("Role deletion message text matching failed. Expected text is ["
					+ ExpdelRoleSuccessMsg
					+ "] and the return text is ["
					+ delRoleSuccessMsg + "]");
		}
	...
	public void verifyLowLevelUserAccessPermissions() {
		...
		if (!Message.equalsIgnoreCase(ErrorMessage)) {
			...
			if (element == null) {
				logger.error("Unable to identify Application selection dropdown in Analytics - Overview page");
				ReportUtils
						.setStepDescription(
								"Unable to identify Application selection dropdown in Analytics - Overview page",
								true);
				m_assert.fail("Unable to identify Application selection dropdown in Analytics - Overview page");
			}
			...
			logger.error("The given Email address is already taken. Please use another Email");
			ReportUtils
					.setStepDescription(
							"The given Email address is already taken. Please use another Email",
							true);
			m_assert.fail("The given Email address is already taken. Please use another Email");
		}
		m_assert.assertAll();
	}
